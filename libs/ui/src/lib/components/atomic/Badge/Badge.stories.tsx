...
import { View, Icon, Button, Avatar } from '..';
...
export const rounded = () => (
  <Example>
    ...
    <Example.Item
      title={[
        'rounded, all sizes, color: critical',
        'one character, renders as circle',
      ]}
    >
      <View direction="row" gap={3}>
...
export const container = () => {
  ...
  return (
    <Example
      title={<Button onClick={() => setHidden(!hidden)}>Toggle badges</Button>}
    >
      ...
      <Example.Item
        title={[
          'position: top-end, overlap',
          'should cover the circular avatar',
        ]}
      >
      ...
      <Example.Item
        title={['position: top-end, overlap', 'should cover the icon']}
      >
        <Badge.Container overlap position="top-end">
          <Badge size="small" color="primary" rounded hidden={hidden} />
          <Icon svg={IconX} size={5} />
        </Badge.Container>
      </Example.Item>
    </Example>
  );
